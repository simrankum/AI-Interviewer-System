  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-2 bg-gradient-to-br from-slate-50 via-slate-100 to-orange-50 relative">
      ...
      <div className="absolute inset-0 overflow-hidden">
        {/* Grid pattern overlay - represents "Grid" in Grid Dynamics */}
      ...
      <div className="w-full max-w-md bg-white rounded-2xl shadow-xl px-8 py-10 border border-slate-200 relative z-10">
        <div className="flex flex-col items-center">
          ...
          <div className="text-center mb-6">
            ...
            <p className="text-sm text-gray-500">Sign up for the Talent Acquisition Dashboard</p>
          </div>
        </div>
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            <label className="block mb-1 text-sm font-medium text-gray-700">Name</label>
            <Input type="text" name="name" autoComplete="name" value={values.name} required onChange={handleChange} placeholder="Your name"/>
          </div>
          <div>
          ...
          <div>
            ...
            <Input type="password" name="password" autoComplete="new-password" value={values.password} required onChange={handleChange} placeholder="At least 8 characters"/>
          </div>
          <Button type="submit" className="w-full bg-gradient-to-r from-yellow-500 to-orange-500 hover:from-yellow-600 hover:to-orange-600 text-white font-medium py-2 rounded-md shadow-md transition-all duration-200">Sign Up</Button>
        </form>
        <div className="mt-6 text-center text-sm text-gray-500">
          Already have an account?{" "}
          <Link to="/login" className="text-blue-600 hover:text-indigo-700 font-semibold hover:underline">Login</Link>
        </div>
      </div>
    </div>
  );
